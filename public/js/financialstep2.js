/**
 * Financial Assistance Step 2 Masterlist JavaScript
 */

/**
 * Populates and opens the Quick Preview modal for a selected intake record.
...
function viewIntakeDetails(intake) {
    ...
    // Header info
    const headerEl = document.getElementById('modalControlNumberHeader');
    if (headerEl) {
        headerEl.textContent = 'Control No: ' + (intake.control_number || 'N/A') + ' • Processed: ' + (intake.date_processed ? intake.date_processed.split('T')[0] : 'N/A');
    }
    ...
    const benAddress = [
        intake.beneficiary_street_address,
        intake.beneficiary_barangay,
        intake.beneficiary_city || 'Silang',
        intake.beneficiary_province || 'Cavite'
    ].filter(Boolean).join(', ') || 'N/A';
    ...
    const benSalaryEl = document.getElementById('modalBeneficiarySalary');
    if (benSalaryEl) {
        benSalaryEl.textContent = intake.beneficiary_monthly_salary
            ? '₱' + parseFloat(intake.beneficiary_monthly_salary).toLocaleString('en-US', { minimumFractionDigits: 2 })
            : 'N/A';
    }
    ...
    if (repCard) {
        if (intake.has_representative) {
            ...
            const repFullName = [
                intake.rep_first_name,
                intake.rep_middle_name,
                intake.rep_last_name,
                intake.rep_extension_name
            ].filter(Boolean).join(' ') || 'N/A';
            ...
            const repAddress = [
                intake.rep_street_address,
                intake.rep_barangay,
                intake.rep_city || 'Silang',
                intake.rep_province || 'Cavite'
            ].filter(Boolean).join(', ') || 'N/A';
            ...
            const repSalaryEl = document.getElementById('modalRepSalary');
            if (repSalaryEl) {
                repSalaryEl.textContent = intake.rep_monthly_salary
                    ? '₱' + parseFloat(intake.rep_monthly_salary).toLocaleString('en-US', { minimumFractionDigits: 2 })
                    : 'N/A';
            }
        } else {
    ...
    const assessmentEl = document.getElementById('modalSocialWorkerAssessment');
    if (assessmentEl) {
        assessmentEl.textContent = intake.social_worker_assessment || 'Assessment completed in Step 1 intake.';
    }
