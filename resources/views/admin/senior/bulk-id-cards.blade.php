            ...
            <div class="detail-header-top">
                <div class="case-info">
                    <h1 id="headerControlNo">
                        ...
                        @else
                            BULK-ID-{{ date('Y-m') }}-{{ str_pad(count($cardData), 4, '0', STR_PAD_LEFT) }}
                        @endif
                    </h1>
                ...
                <div class="header-actions">
                    ...
                    <button type="button" class="header-btn primary" style="width:120px;flex-shrink:0;" onclick="reprintBulkCards()">
                        <i data-lucide="printer" style="width:16px;height:16px;"></i>
                        Reprint
                    </button>
                                ...
                                <div class="card-pair" id="cardPair_{{ $card['senior']->id }}">
                                    ...
                                    <div class="id-card">
                                        ...
                                        <div class="emergency-section">
                                            ...
                                            <div class="details">
                                                <div class="field"><span class="label">Name:</span><span class="value" id="cardEmergencyName_{{ $card['senior']->id }}">{{ $card['senior']->emergency_contact_name ?? '' }}</span></div>
                                                <div class="field"><span class="label">Contact:</span><span class="value" id="cardEmergencyContact_{{ $card['senior']->id }}">{{ $card['senior']->emergency_contact_number ?? '' }}</span></div>
                                                <div class="field"><span class="label">Address:</span><span class="value" id="cardEmergencyAddress_{{ $card['senior']->id }}">{{ $card['senior']->emergency_contact_relationship ?? '' }}</span></div>
                                            </div>
                                        </div>
...
<script>
    ...
    function reprintBulkCards() {
        Swal.fire({
            title: 'Reprint Bulk ID Cards',
            text: `Are you sure you want to reprint all ${seniorIds.length} ID card(s)?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                fetch(`{{ route('admin.senior.bulk-reprint-ids') }}`, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({ ids: seniorIds })
                })
                .then(r => r.json())
                .then(data => {
                    window.print();
                    Swal.fire({
                        ...
                        text: `All ${seniorIds.length} ID card(s) have been recorded and sent to printer.`,
                        icon: 'success',
                        confirmButtonColor: '#1A237E',
                        confirmButtonText: 'OK'
                    });
