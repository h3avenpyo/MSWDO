...
<div class="container-fluid px-0">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                ...
                <p class="text-white-50 small mb-0">
                    Monitor, summarize, and liquidate financial assistance payrolls <strong>per month</strong>. Track
                    monthly allocated funds, claimed disbursements, and remaining balances in real time.
                </p>
            </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="stat-metric-card card-allocated d-flex align-items-center gap-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <h4 class="fw-bold mb-0 text-dark" id="global-total-allocated"
                        data-raw-amount="{{ $globalTotalAllocated }}">{{ $formattedGlobalAllocated }}</h4>
                    <div class="text-muted text-2xs mt-0.5">
                        Across {{ $totalMonthsCount }} {{ Str::plural('month', $totalMonthsCount) }} ({{
                        $globalTotalBeneficiaries }} clients)
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="stat-metric-card card-claimed d-flex align-items-center gap-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <h4 class="fw-bold mb-0 text-success" id="global-total-claimed"
                        data-raw-amount="{{ $globalTotalClaimed }}">{{ $formattedGlobalClaimed }}</h4>
                    <div class="text-muted text-2xs mt-0.5">
                        <span id="global-claimed-count">{{ $globalClaimedCount }}</span> {{ Str::plural('beneficiary',
                        $globalClaimedCount) }} released
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="stat-metric-card card-remaining d-flex align-items-center gap-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <h4 class="fw-bold mb-0 text-warning-emphasis" id="global-total-unclaimed"
                        data-raw-amount="{{ $globalTotalUnclaimed }}">{{ $formattedGlobalUnclaimed }}</h4>
                    <div class="text-muted text-2xs mt-0.5">
                        <span id="global-unclaimed-count">{{ $globalUnclaimedCount }}</span> {{
                        Str::plural('beneficiary', $globalUnclaimedCount) }} pending
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-6 col-lg-3">
            <div class="stat-metric-card card-remaining d-flex align-items-center gap-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <h4 class="fw-bold mb-0 text-dark" id="global-remaining-balance"
                        data-raw-amount="{{ $globalRemainingBalance }}">{{ $formattedGlobalRemaining }}</h4>
                    <div class="text-muted text-2xs mt-0.5">
                        Rate: <strong class="text-success" id="global-liquidation-rate">{{ $globalLiquidationRate
                            }}%</strong> liquidated
                    </div>
                </div>
            </div>
    ...
    <div class="filter-card mb-4">
        <form id="liquidationFilterForm" action="{{ route('admin.financial.financialstep2.liquidation') }}" method="GET"
            class="row g-2 align-items-end">
            <div class="col-md-5 col-lg-5">
                <label class="form-label small fw-bold text-muted mb-1"><i class="fas fa-search me-1"></i> Search
                    Payroll Reference or Beneficiary</label>
                <input type="text" id="liquidationSearchInput" name="search"
                    class="form-control form-control-sm rounded-3"
                    placeholder="Search by Control No, Beneficiary, Payroll Reference..."
                    value="{{ request('search') }}" autocomplete="off">
            </div>
            <div class="col-md-3 col-lg-3">
                <label class="form-label small fw-bold text-muted mb-1"><i class="fas fa-calendar-alt me-1"></i> Filter
                    by Month</label>
            ...
            <div class="col-md-2 col-lg-2">
                <label class="form-label small fw-bold text-muted mb-1"><i class="fas fa-filter me-1"></i> Liquidation
                    Status</label>
                <select name="status" class="form-select form-select-sm rounded-3">
                    <option value="All">All Statuses</option>
                    <option value="fully_liquidated" {{ request('status')==='fully_liquidated' ? 'selected' : '' }}>
                        Fully Liquidated (100%)</option>
                    <option value="partially_liquidated" {{ request('status')==='partially_liquidated' ? 'selected' : ''
                        }}>Partially Liquidated</option>
                    <option value="unreleased" {{ request('status')==='unreleased' ? 'selected' : '' }}>Unreleased (0%)
                    </option>
                </select>
            </div>
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap mt-2 pt-2 border-top">
            ...
            <span class="badge bg-light text-dark border rounded-pill px-2.5 py-1 text-xs">
                ...
                <a href="{{ route('admin.financial.financialstep2.liquidation', request()->except('search')) }}"
            ...
            <span class="badge bg-light text-dark border rounded-pill px-2.5 py-1 text-xs">
                ...
                <a href="{{ route('admin.financial.financialstep2.liquidation', request()->except('month')) }}"
                    class="text-muted ms-1 text-decoration-none">&times;</a>
            </span>
            @endif
            ...
            <span class="badge bg-light text-dark border rounded-pill px-2.5 py-1 text-xs">
                ...
                <a href="{{ route('admin.financial.financialstep2.liquidation', request()->except('status')) }}"
                    class="text-muted ms-1 text-decoration-none">&times;</a>
            </span>
            @endif
            <a href="{{ route('admin.financial.financialstep2.liquidation') }}"
                class="text-danger small text-decoration-none ms-1">Reset all</a>
        </div>
        @endif
    </div>
        ...
        <div class="payroll-card-header p-3.5">
                ...
                <div class="d-flex align-items-center gap-3 flex-wrap flex-grow-1">
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <span
                                class="badge bg-secondary-subtle text-secondary border border-secondary-subtle rounded-pill px-2.5 py-0.5 text-xs">
                                <i class="fas fa-layer-group me-1"></i> {{ $month->payrolls_count }} {{
                                Str::plural('Payroll Batch', $month->payrolls_count) }}
                            </span>
                        ...
                        <div class="text-muted small mt-1">
                            ...
                            (<span class="text-success fw-semibold" id="month-claimed-count-{{ $month->month_key }}">{{
                                $month->claimedCount }}</span> Claimed /
                            <span class="text-warning-emphasis fw-semibold"
                                id="month-unclaimed-count-{{ $month->month_key }}">{{ $month->unclaimedCount }}</span>
                            Unclaimed)
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-2 mt-3 pt-3 border-top">
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="finance-metric-box">
                        ...
                        <div class="h5 fw-bold mb-0 text-dark" id="month-allocated-{{ $month->month_key }}"
                            data-raw-amount="{{ $month->totalAllocated }}">
                            {{ $month->formattedAllocated }}
                        </div>
                        <div class="text-muted text-2xs mt-0.5">Allocated for {{ $month->month_label }} ({{
                            $month->totalBeneficiariesCount }} clients)</div>
                    </div>
                </div>

                <!-- 2. Total Claimed Amount -->
                <div class="col-sm-6 col-md-3">
                    <div class="finance-metric-box border-success-subtle">
                        <div class=" small text-uppercase fw-bold text-2xs">Total Claimed Amount</div>
                        <div class="h5 fw-bold mb-0 " id="month-claimed-{{ $month->month_key }}"
                            data-raw-amount="{{ $month->totalClaimed }}">
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="finance-metric-box ">
                        <div class="small text-uppercase fw-bold text-2xs">Total Unclaimed Amount
                        </div>
                        <div class="h5 fw-bold mb-0 " id="month-unclaimed-{{ $month->month_key }}"
                            data-raw-amount="{{ $month->totalUnclaimed }}">
                ...
                <div class="col-sm-6 col-md-3">
                    <div class="finance-metric-box border-warning-subtle" style="background-color: #FFFDF5;">
                        ...
                        <div class="h5 fw-bold mb-0 text-dark" id="month-remaining-{{ $month->month_key }}"
                            data-raw-amount="{{ $month->remainingBalance }}">
            ...
            <div class="mt-3">
                <div class="d-flex justify-content-between align-items-center small mb-1">
                    ...
                    <span class="fw-bold text-dark text-xs" id="month-progress-text-{{ $month->month_key }}">{{
                        $month->liquidationRate }}% Liquidated</span>
                </div>
                <div class="progress" style="height: 8px;">
                    <div id="month-progress-{{ $month->month_key }}"
                        class="progress-bar bg-{{ $month->liquidationStatusClass === 'warning' ? 'warning' : ($month->liquidationStatusClass === 'success' ? 'success' : 'secondary') }} progress-bar-liquidation"
                        role="progressbar" style="width: {{ $month->liquidationRate }}%;"
                        aria-valuenow="{{ $month->liquidationRate }}" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
            </div>
        </div>
    </div>
