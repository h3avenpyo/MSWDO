...
class SocialCaseIntakeController extends Controller
{
    /**
     * Display a listing of beneficiary intakes for Social Case Studies.
     */
    ...
    /**
     * Show the form for creating a new intake sheet in Social Case Module.
     */
    public function create(EligibilityChecker $checker, ?Client $client = null)
    {
        if ($client && ! $checker->check($client)['eligible']) {
            return redirect()->route('admin.social-case-eligibility.show', $client)
                ->with('error', 'This client is not eligible to proceed to social case intake.');
        }
    ...
    /**
     * Store a newly created social case intake sheet in database.
     */
    public function store(StoreBeneficiaryIntakeRequest $request, EligibilityChecker $checker)
    {
        ...
        $data['service_provided'] = $data['service_provided'] ?? 'Social Case Study Intake';
        $data['purpose'] = $data['purpose'] ?? ($data['assistance_purpose'] ?? 'Social Welfare Case Request');
        ...
        if ($client && ! $checker->check($client)['eligible']) {
            return redirect()->route('admin.social-case-eligibility.show', $client)
                ->with('error', 'This client is not eligible to proceed to social case study creation.');
        }

        $intake = BeneficiaryIntake::create($data);

        return redirect()->route('admin.social-case.intake.show', $intake)
            ->with('success', 'Social Case Intake Sheet has been successfully saved.');
    }
    ...
    /**
     * Show the form for editing the specified social case intake sheet.
     */
    ...
    /**
     * Update the specified intake sheet in database for Social Case Module.
     */
    public function update(UpdateBeneficiaryIntakeRequest $request, BeneficiaryIntake $intake)
    {
        ...
        return redirect()->route('admin.social-case.intake.show', $intake)
            ->with('success', 'Social Case Intake Sheet has been updated successfully.');
    }
    ...
    public function destroy(BeneficiaryIntake $intake)
    {
        ...
        return redirect()->route('admin.social-case.intake.index')
